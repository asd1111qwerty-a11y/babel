...
function GameServer() {
    ...
    this.muted = []; // List of muted player names
...
GameServer.prototype.getCachedRole = function (username, callback) {
    var now = Date.now();
    var cached = this.roleCache[username];

    // Return cache kalau masih valid
    if (cached && cached.expiry > now) {
        return callback(cached.data);
    }

    // Fetch dari backend kalau cache expired
    var http = require('http');
    var self = this;
    http.get('http://localhost:3001/role/' + encodeURIComponent(username), function (res) {
        var body = '';
        ...
        res.on('end', function () {
            try {
                ...
                // Simpan ke cache
                self.roleCache[username] = { data: data, expiry: now + self.CACHE_TTL };
                callback(data);
            } catch (e) { callback({ role: 'user', color: null }); }
        });
    }).on('error', function () { callback({ role: 'user', color: null }); });
};

GameServer.prototype.getCachedGuildId = function (username, callback) {
    var now = Date.now();
    var cached = this.guildCache[username];

    if (cached && cached.expiry > now) {
        return callback(cached.guild_id);
    }

    var http = require('http');
    var self = this;
    http.get('http://localhost:3001/guildid/' + encodeURIComponent(username), function (res) {
        var body = '';
        res.on('data', function (c) { body += c; });
        res.on('end', function () {
            try {
                var data = JSON.parse(body);
                self.guildCache[username] = { guild_id: data.guild_id, expiry: now + self.CACHE_TTL };
                callback(data.guild_id);
            } catch (e) { callback(null); }
        });
    }).on('error', function () { callback(null); });
};

// Tambah fungsi clear cache (dipanggil saat player disconnect/pindah guild)
GameServer.prototype.clearPlayerCache = function (username) {
    delete this.roleCache[username];
    delete this.guildCache[username];
    delete this.chatColorCache[username]; // ← TAMBAH INI
};

GameServer.prototype.getCachedChatColor = function (username, callback) {
    var now = Date.now();
    var cached = this.chatColorCache[username];
    if (cached && cached.expiry > now) return callback(cached.color);

    var http = require('http');
    var self = this;
    http.get('http://localhost:3001/chatcolor/' + encodeURIComponent(username), function (res) {
        var body = '';
        res.on('data', function (c) { body += c; });
        res.on('end', function () {
            try {
                var color = JSON.parse(body).chat_color || null;
                self.chatColorCache[username] = { color: color, expiry: now + self.CACHE_TTL };
                callback(color);
            } catch (e) { callback(null); }
        });
    }).on('error', function () { callback(null); });
...
GameServer.prototype.start = function () {
    ...
    this.chatLogStream = fs.createWriteStream('logs/chat.log', { flags: 'a' });
    ...
    function connectionEstablished(ws) {
        // Set real IP paling awal sebelum apapun
        ws.remoteAddress = ws.upgradeReq.headers['x-real-ip']
            || (ws.upgradeReq.headers['x-forwarded-for'] || '').split(',')[0].trim()
        ...
        if (this.clients.length >= this.config.serverMaxConnections) {
            ...
            ws.close();
            return;
        } else if (this.banned.indexOf(ws.remoteAddress) != -1) { // pakai ws.remoteAddress
        ...
        if (this.config.serverMaxConnPerIp) {
            for (var cons = 1, i = 0, llen = this.clients.length; i < llen; i++) {
                if (this.clients[i].remoteAddress == ws.remoteAddress) { // pakai ws.remoteAddress
                    cons++;
                }
            }
        ...
        function close(error) {
            ...
            var playerName = client && client.name ? client.name : null;
            if (playerName) {
                playerName = playerName.replace(/^[\x01-\x05]/, '');
                if (playerName.indexOf('] ') !== -1) playerName = playerName.split('] ')[1] || playerName;
                this.server.clearPlayerCache(playerName);
            }
        ...
        // ws.remoteAddress dan ws.remotePort sudah diset di atas, tidak perlu diset lagi
...
GameServer.prototype.spawnPlayer = function (player, pos, mass) {
    ...
    if ('_socket' in player.socket) {
        ...
        var displayZname = zname.indexOf('\n') !== -1 ? zname.split('\n')[1] : zname;
        console.log("\u001B[36m" + displayZname + " joined the game\u001B[0m");
    }
...
GameServer.prototype.splitCells = function (client) {
    ...
    for (var i = 0; i < len; i++) {
        ...
        split.setMoveEngineData(50 + (cell.getSpeed() * 2.5), 13);
        split.calcMergeTime(this.config.playerRecombineTime);
        split.firstSplit = true;
        setTimeout(function () { split.firstSplit = false; }, 100)
        /* split.hasAte = true;
...
GameServer.prototype.MasterPing = function () {
    this.getStats(); // Pastikan stats selalu fresh
    try {
