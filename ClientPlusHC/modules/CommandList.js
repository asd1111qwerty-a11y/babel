...
Commands.list = {
    help: function (gameServer, split) {
        ...
        console.log("\u001B[36mmute       \u001B[0m: mute a player by name");
        console.log("\u001B[36munmute     \u001B[0m: unmute a player by name");
        console.log("\u001B[36mmutelist   \u001B[0m: show muted players");
        console.log("============================================================");
    },
    ...
    mute: function (gameServer, split) {
        ...
        if (!name) {
            console.log("\u001B[36mServer: \u001B[0mPlease specify a player name!");
            return;
        }
        if (gameServer.muted.indexOf(name.toLowerCase()) !== -1) {
            console.log("\u001B[36mServer: \u001B[0m" + name + " is already muted");
            return;
        }
        gameServer.muted.push(name.toLowerCase());
        console.log("\u001B[36mServer: \u001B[0mMuted " + name);
    },

    unmute: function (gameServer, split) {
        var name = split.slice(1).join(' ');
        if (!name) {
            console.log("\u001B[36mServer: \u001B[0mPlease specify a player name!");
            return;
        }
        var idx = gameServer.muted.indexOf(name.toLowerCase());
        if (idx === -1) {
            console.log("\u001B[36mServer: \u001B[0m" + name + " is not muted");
            return;
        }
        gameServer.muted.splice(idx, 1);
        console.log("\u001B[36mServer: \u001B[0mUnmuted " + name);
    },
    mutelist: function (gameServer, split) {
        console.log("\u001B[36mServer: \u001B[0mMuted players (" + gameServer.muted.length + "):");
        gameServer.muted.forEach(function (n) { console.log("  " + n); });
    },
    ...
    mass: function (gameServer, split) {
        // Cek apakah split[1] adalah angka (self-mass) atau nama (target)
        var name, amount;
        if (!isNaN(parseInt(split[1])) && typeof split[2] == 'undefined') {
            // /mass 500 -> self, tapi CommandList ga tau siapa sender
            // nama di-pass dari PacketHandler via split[0] = 'mass', split[1] = nama, split[2] = amount
            // fallback: skip, handle di bawah
            name = '__self__';
            amount = Math.max(parseInt(split[1]), 9);
        } else {
            name = split[1];
            amount = Math.max(parseInt(split[2]), 9);
        }

        if (typeof name == 'undefined') {
            console.log("\u001B[36mServer: \u001B[0mPlease specify a player name or amount!");
            return;
        }
        if (isNaN(amount)) {
        ...
        for (var i in gameServer.clients) {
            ...
            if (cleanName.indexOf('\n') !== -1) cleanName = cleanName.split('\n')[1];
            cleanName = cleanName.replace(/^[\x01\x02\x03\x04]/, '');
            if (cleanName.indexOf('] ') !== -1) cleanName = cleanName.split('] ')[1] || cleanName;

            if (cleanName.toLowerCase().trim() === name.toLowerCase().trim()) {
                for (var j in client.cells) {
                    client.cells[j].mass = amount;
                }
                console.log("\u001B[36mServer: \u001B[0mSet mass of " + cleanName + " to " + amount);
                return;
            }
        }

        console.log("\u001B[36mServer: \u001B[0mPlayer '" + name + "' was not found");
    },
    merge: function (gameServer, split) {
        ...
        for (var i in gameServer.clients) {
            ...
            if (cleanName.toLowerCase().trim() === name.toLowerCase().trim()) {
                ...
                console.log("\u001B[36mServer: \u001B[0mForced " + cleanName + " to merge");
                return;
            }
        }
    ...
    say: function (gameServer, split) {
        ...
        // Kirim sebagai chat [SERVER] warna hijau
        var nick = '[SERVER]';
        var buf = new ArrayBuffer(9 + 2 * nick.length + 2 * message.length);
        var view = new DataView(buf);
        view.setUint8(0, 99);
        view.setUint8(1, 0);
        view.setUint8(2, 255);  // r
        view.setUint8(3, 50);   // g
        view.setUint8(4, 50);   // b
        var offset = 5;
        ...
        for (var j = 0; j < message.length; j++) {
            view.setUint16(offset, message.charCodeAt(j), true);
            offset += 2;
        }
        view.setUint16(offset, 0, true);
        for (var i = 0; i < gameServer.clients.length; i++) {
            gameServer.clients[i].send(buf);
        }
